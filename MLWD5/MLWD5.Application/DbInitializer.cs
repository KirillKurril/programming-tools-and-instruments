    ...
    public static class DbInitializer
    {
        public static async Task Initialize(IServiceProvider services)
        {
            ...
            for (int i = 1; i <= 10; i++)
            {
                ...
                singer.SetId(i);
                singer.SetPhotoSource("C:/Uni/programming-tools-and-instruments/MLWD5/MLWD5.UI/Resources/Images/singer.png");

                for (int j = 1; j <= 3; j++)
                {
                    var song = new Song($"Song {j} by Singer {i}", $"Description {j}", $"Text {j}", i + j);
                    song.SetId(i * 100 + j);
                    song.SetPhotoSource("C:/Uni/programming-tools-and-instruments/MLWD5/MLWD5.UI/Resources/Images/song.jpg");
                    song.AddToSinger(i);
